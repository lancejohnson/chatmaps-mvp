...
PARCELS_SCHEMA = {
    ...
    'search_columns': ['apn', 'situs_street_name', 'situs_house_number', 'situs_city_name'],
    'columns': {
        'ogc_fid': {
            'type': 'integer',
        ...
        'wkb_geometry': {
            ...
            'nullable': True,
            'good_for': ['spatial_queries', 'mapping', 'area_calculations', 'geometric_analysis'],
...
"""
    ...
    for col_name, col_info in table_schema['columns'].items():
        if col_name not in search_cols and col_name != geom_col:
            if 'user_queries' in col_info.get('good_for', []) or 'analysis' in col_info.get('good_for', []):
                context += f"- {col_name}: {col_info.get('description', 'No description')}\n"
